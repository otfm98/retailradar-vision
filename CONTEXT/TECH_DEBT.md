...
*Real problems knowingly deferred, not aspirational improvements. Each item is something we're choosing not to fix right now, with enough detail that fixing it later doesn't require re-discovering the problem.*

## Index

| ID | Severity | Area | Status | Description |
|---|---|---|---|---|
| BACKEND-01 | HIGH | Backend | OPEN | No backend exists — all data access is direct client-side fetch |
| BACKEND-02 | HIGH | Backend | OPEN | No env config / secret storage mechanism |
...
All three live integrations (`src/lib/liveData.ts`: ABS CPI, QLD Open Data, Gold Coast ArcGIS) are called directly from the browser. This works today only because all three happen to be public, CORS-permissive, and keyless (or client-safe). It structurally cannot extend to TRA, economy.id, CBRE/NAB, or POS data, which need credentials the client should never hold, and it can't host a forecasting model.
...
**Suggested fix:** Stand up a minimal backend (even a single serverless function layer) that proxies external calls and will eventually host the forecasting model. Design it before wiring in any credentialed data source.

### BACKEND-02 — No env config / secret storage
**Severity:** HIGH · **Owner:** unassigned · **Status:** OPEN

No `.env` files, no `import.meta.env` usage found in app code. There is currently nowhere to safely put an API key even if one were needed.

**Why it matters:** Every credentialed integration (TRA, POS, and possibly ABS depending on DATA-02) is blocked until this exists.

**Suggested fix:** Establish env config conventions alongside BACKEND-01 — this is really the same piece of work, not a separate one.
...
No test files, no test runner configured in `package.json`.
...
**Suggested fix:** Pick a test runner (Vitest is the natural fit given the Vite-based toolchain) when backend work starts, not before it's needed.
